...
[source,clojure]
----
include::../src/com/wsscode/pathom/book/graphql/fulcro_network/name_query.cljs[]
----
...
<3> The viewer goes on the root query
<4> Trigger the load for the `:viewer`
<5> Setup the GraphQL network, it's a Fulcro network that does the query translation automatically

You can try this example here, just get yourself a github personal token.

++++
<div x-app="demo-fulcro-network-name-query"></div>
++++
