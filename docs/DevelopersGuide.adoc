...
the Om Next documentation and in the http://book.fulcrologic.com/#_the_query_and_mutation_language[Fulcro Developer's Guide]

For now, you can get by with just a short introduction:

== Query Notation Introduction

We're assuming you know something about Om Next's query notation, but for introduction
purposes, here's a short summary:
...
If you want to obtain the name and address:

```
[:person/name :person/age] ; Get properties name and address
```

If you want to obtain a name, and the names of children:

```
[:person/name {:person/address [:address/street]}]
```

The result of running such a query is a map containing the result (in the same shape as the query):
...
*against* a given context). You can't ask for `:person/name` without knowing which entity (person) you're asking it of.
...
In our earlier example of running `[:person/name {:person/address [:address/street]}]` against "Sam", we get:

```
{:person/name "Sam" :person/address {:address/street "111 Main St."}}
```

but how did it know what to run the subquery `[:address/street]` against? It used's Sam's address, of course!
...
When parsing you supply the initial environment which also establishes the context of the query. The environment is a map that can contain anything you wish, and can be seen in any code that you plug in to process the query. There are some predefined (namespaced) keys that have special meaning to the parser. In particular `:com.wsscode.pathom.core/reader` can be used to supply a reader function for the parser to use.
...
Before we continue, I would like to talk to about some patterns on the graph parsing game, it will give you a better
understanding of how/why this library is designed the way it is. When parsing a graph API like this, there are 3 major
types of reading that you want to do at any level, let's talk about those:

. *Entity attributes*: Attributes that are present on the current entity (node) that is being processed during the parser (the current context). For example, if we are at a `customer` node, it might have attributes like `:customer/id` and `:customer/name`. When the query asks for those we should fetch them from the entity itself.
...
.. *Derived attributes*: when the computed attribute depends on some property of the current entity, then we call it a derivation. Derivations are often relationship mappings, like navigating a join on `:customer/address` or `:customer/friends`. They may also be a transformation of real data from the contextual entity.
. *Entity lookups*: This is the om.next default way to look for a specific entity on the graph using the ident syntax (eg: `[:customer/id 123]`). Using such a literal entity lookup is a way of setting the context. It is essentially a "goto entity" for graph notation.
...
A vector of plugins can be added when the parser is created. The `p/env-plugin` automatically adds the given map of data to the parsing environment every time the resulting parser is called.
...
Here's a quick summary of things for quick reference:
...
`p/map-reader`:: A reader that assumes the current contextual entity is a map, and tries to use it to satisfy queries
...
Entities can also be atoms, this is a useful caching mechanism that allows for a reader to add information into the current entity without having to make a join.

The built-in function `p/entity` always returns a Clojure map, if the entity is an atom it will deref it automatically.

Let's see an example using an entity atom:

```clojure
include::../src/com/wsscode/pathom/book/entities/atom_entities.cljc[]
```

You can think atom entities as a narrow type of caching, valid only for the processing of the current entity.
...
[source,clojure]
----
include::../src/com/wsscode/pathom/book/graphql/fulcro_network/github_name.cljs[]
----
...
[source,clojure]
----
include::../src/com/wsscode/pathom/book/graphql/fulcro_network/github_latest_stars.cljs[]
----
...
[source,clojure]
----
include::../src/com/wsscode/pathom/book/graphql/fulcro_network/github_mutation_stars.cljs[]
----
