...
You can get a basic parser using:
...
please add `public_repo` scope when you generate it, so you can try all the demo features.
...
What were the last projects you Github that you gave a star to? We can ask the Github
GraphQL with this query:
...
type Group @model {
  ...
  updatedAt: DateTime!
  name: String
  contacts: [Contact!]! @relation(name: "GroupOnContact")
}
...
type Contact @model {
  ...
  id: ID! @isUnique
  updatedAt: DateTime!
  github: String @isUnique
  groups: [Group!]! @relation(name: "GroupOnContact")
}
----

++++
<div xx-app="contacts"></div>
++++
